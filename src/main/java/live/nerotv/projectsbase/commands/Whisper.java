...
implements CommandExecutor {
    public boolean onCommand(@NotNull CommandSender s, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        if (s == null) {
            Whisper.$$$reportNull$$$0(0);
        }
        if (cmd == null) {
            Whisper.$$$reportNull$$$0(1);
        }
        if (label == null) {
            Whisper.$$$reportNull$$$0(2);
        }
        if (args == null) {
            Whisper.$$$reportNull$$$0(3);
        }
        if (s instanceof Player) {
            ...
            if (PlayerAPI.isRP(p)) {
                if (PlayerAPI.getRPN(p.getUniqueId().toString()) == null) {
                    ...
                    API.sendErrorMessage((CommandSender)p, "\u00a74Fehler: \u00a7c/whisper [Nachricht]");
                } else {
                    Object MSG = "";
                    ...
                    String RPM = "\u00a76RP \u00a78\u25cf \u00a7e" + Job2 + "\u00a78 \u25cf \u00a76" + PlayerAPI.getRPN(p.getUniqueId().toString()) + "\u00a78 (fl\u00fcstert) \u00bb \u00a7f" + ((String)MSG).toLowerCase();
                    for (Player all : Bukkit.getOnlinePlayers()) {
                        if (!p.getLocation().getWorld().equals((Object)all.getLocation().getWorld()) || !(p.getLocation().distance(all.getLocation()) <= 6.0)) continue;
    ...
    private static  void $$$reportNull$$$0(int n) {
        ...
        arrobject[1] = "live/nerotv/projectsbase/commands/Whisper";
